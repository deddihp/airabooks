...
//echo "QUERY = ".$query;
	...
	if ( $random_code != "" ) {
		...
		");
		return "<p class=\"about\">Kode aktivasi telah dikirim ke Email anda yang terdaftar di airabooks.</p><p class=\"about\">
	...
	function synchronize(&$fb_id) {
		...
		$query = "select random_code, fb_id from user_activation where subscriber_id='".$idsubs."'";
		//echo "QUERY = ".$query;
		$mysql = new MySQLComic($GLOBALS['COMIC_DBWEB']);
		$result = $mysql->query($query);
		$row = mysqli_fetch_array($result);
		if ( $row['random_code'] != $activation_code ) {
			return "<p class=\"about\">Kode aktivasi anda tidak cocok, Silakan menghubungi admin airabooks</p>";
		}
