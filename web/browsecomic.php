...
<html>
	<head>
		<title>Browse Komik</title>
	
		<?
			$description = "Browse Komik, Lihat Komik";
	...
	<body>
		...
		?>
		<?
			$menu = $mlayout->writeMenu('Browse Komik', "");
		?>
		<?
		$mcontent = "
		<div class=\"mcontent\" id=\"mcontent\" >
			...
			<p class=\"left\">Koleksi Komik</p>";
			
				$local_search = $_GET['local_search'];
				$cur_offset = $_GET['offset'];
				$cur_offset = ($cur_offset=="")?0:$cur_offset;
				$content = $bookscommon->writeBooksGroup($cur_offset, 20, $local_search);
				$numrows = $bookscommon->getBooksNumRows($local_search);	
			?>
			<?
				...
				$action_handler = "browsecomic.php";
			?>
			<?
			$nav_var = "
			...
			<div style=\"display:table; margin:0px auto; border:0px solid black;\">
				<form  class=\"form_style\" name=\"local_search\" method=\"get\" action=\"browsecomic.php\">
